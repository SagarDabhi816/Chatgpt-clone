...
const ChatMessages = ({ messages, isSending }) => {
  ...
  return (
    <div className="messages mt-10" aria-live="polite">
      {messages.map((m,index) => (
        <div key={index} className={`msg msg-${m.type}`}>
          {/* <div className="msg-role bg-red-400" aria-hidden="true">{m.type === 'user' ? 'You' : 'AI'}</div> */}
          <div className={`${m.type === 'user' ? 'bg-[#2A2A2A]!' : '#2A2A2A'} msg-bubble `}>{m.content} </div>
